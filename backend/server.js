...
// Import MRI analysis routes
const mriAnalysisRoutes = require('./routes/mri_analysis');
...
// Serve uploaded files (MRI images, etc.)
...
app.get('/api/chat/conversations/:withUserId/messages', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      conversationId: convo._id, messages: msgs.map(m => ({
        id: m._id,
...
// Upload media (image/audio/document) to Cloudinary or local storage
app.post('/api/chat/upload-media', authenticateToken, async (req, res) => {
  try {
    const { dataUri, resourceType = 'auto' } = req.body;
    if (!dataUri) return res.status(400).json({ message: 'dataUri required' });

    // For documents: save to local filesystem (bypasses Cloudinary's 10MB raw limit)
    if (resourceType === 'raw') {
      const fs = require('fs');
      ...
      console.log(`Document saved locally: ${filePath} (${buffer.length} bytes, ${mime})`);

      // Return URL relative to the static file server
      const fileUrl = `/uploads/documents/${filename}`;
      return res.json({ url: fileUrl, format: ext, resourceType: 'raw' });
    }

    // For images/audio: use Cloudinary as before
    const result = await cloudinary.uploader.upload(dataUri, {
      folder: 'neurocare_chat',
      resource_type: resourceType
    });

    res.json({ url: result.secure_url, format: result.format, resourceType: result.resource_type });
  } catch (error) {
...
// Deprecated: kept for backward compatibility if needed, but frontend should move to upload-media
...
app.post('/api/appointments/book-with-payment', authenticateToken, async (req, res) => {
  try {
    ...
    // Check if doctor has consultation fee set
    console.log('Doctor consultation fee:', doctor.consultationFeeInRupees, 'Type:', typeof doctor.consultationFeeInRupees);
    if (!doctor.consultationFeeInRupees || doctor.consultationFeeInRupees <= 0) {
      return res.status(400).json({ message: 'Doctor has not set a consultation fee' });
    }
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      // If payment creation fails, delete the appointment
    ...
    try {
      ...
      cp = await ConsultationPayment.create({
        patientId: req.user.userId,
        doctorId: doctor._id,
      ...
      return res.status(400).json({ message: paymentError.message || 'Failed to create payment order' });
    }
...
// MRI Analysis Routes
app.use('/api/mri', mriAnalysisRoutes);
