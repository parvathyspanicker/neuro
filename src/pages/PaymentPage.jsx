...
export default function PaymentPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { data, error: bookingError } = await mongodbService.createAppointmentWithPayment(paymentPayload);
      
      console.log('Appointment creation response:', { data, error: bookingError });
      
      if (bookingError) {
        console.error('Booking error details:', bookingError);
        throw new Error(bookingError.message || 'Failed to create appointment with payment');
      }
      if (!data?.order?.id) {
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-blue-50 rounded-lg">
                ...
                <span className="text-2xl font-bold text-blue-600">₹{doctorData.fee || doctorData.consultationFeeInRupees}</span>
              </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    <FaCreditCard />
                    Pay ₹{doctorData.fee || doctorData.consultationFeeInRupees}
                  </>
                )}
              </button>
