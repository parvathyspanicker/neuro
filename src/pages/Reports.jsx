  ...
  async uploadMRIScan(file, scanData) {
    try {
      ...
      const response = await fetch(`${this.baseURL}/patient/upload-mri`, {
        method: 'POST',
        headers: {
          ...(localStorage.getItem('mongodb_token') && { Authorization: `Bearer ${localStorage.getItem('mongodb_token')}` })
        },
        body: formData
      });
...
export default function Reports() {
  ...
  const baseUrl = useMemo(() => (import.meta.env.VITE_MONGODB_API_URL?.replace('/api', '') || 'http://localhost:3002'), []);
  ...
  // Handle new report from navigation state
  useEffect(() => {
    if (location.state?.newReport) {
      setNewReport(location.state.newReport);
      setHighlightReportId(location.state.highlightReportId);

      // Add new report to the beginning of the list
      setMriReports(prevReports => [location.state.newReport, ...prevReports]);
            ...
            <div className="space-y-2 px-6">
              {sidebarItems.map((item) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2 text-left rounded-lg transition-colors ${item.id === 'reports'
        ...
        <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <span className="text-green-800 font-medium">
                    ✅ New report created successfully! Your MRI analysis has been saved to your medical records.
                  </span>
                </div>
                <button
                  onClick={() => setNewReport(null)}
          ...
          {!loading && filteredReports.length === 0 && !error && (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-4">No MRI Reports Yet</h3>
              <p className="text-gray-600 mb-8 max-w-md mx-auto">
                You haven't completed any MRI scans yet. Go to the MRI Analysis page to upload and analyze your first scan.
              </p>
              <div className="text-sm text-gray-500">
                <p>To get started:</p>
                <ol className="mt-2 space-y-1 text-left max-w-xs mx-auto">
                  <li>1. Go to MRI Analysis page</li>
                  <li>2. Upload your MRI scan</li>
                  <li>3. Click "View Full Report" after analysis</li>
                  <li>4. Your report will appear here</li>
                </ol>
              </div>
            </div>
          )}
                  ...
                  <div key={report.id} className="bg-white rounded-2xl shadow-lg p-6">
                    <div className="flex items-start gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        {/* Heatmap image */}
                        <div className="mb-4">
                          {(report.heatmapImage || report.imagePath) ? (
                            <img
                              src={(() => {
                                const img = report.heatmapImage || report.imagePath;
                                if (!img) return '';
                                if (img.startsWith('http')) return img;
                                const base = import.meta.env.VITE_MONGODB_API_URL || 'http://localhost:3002';
                                return img.startsWith('/') ? `${base}${img}` : `${base}/uploads/mri/${img}`;
                              })()}
                              alt="MRI Scan"
                            ...
                            <div className="w-full h-48 bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center">
                              <span className="text-gray-400 text-sm">No MRI image available</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => navigate('/full-report', { state: { report } })}
              ...
              <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    {(selectedReport.heatmapImage || selectedReport.imagePath) ? (
                      <img
                        src={(() => {
                          const img = selectedReport.heatmapImage || selectedReport.imagePath;
                          if (!img) return '';
                          if (img.startsWith('http')) return img;
                          const base = import.meta.env.VITE_MONGODB_API_URL || 'http://localhost:3002';
                          return img.startsWith('/') ? `${base}${img}` : `${base}/uploads/mri/${img}`;
