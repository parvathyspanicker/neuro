...
class MongoDBService {
  ...
  async uploadMedia(dataUri, resourceType = 'auto') {
    try {
      const response = await fetch(`${API_URL}/chat/upload-media`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
        body: JSON.stringify({ dataUri, resourceType })
      });
      const data = await this.parseJsonSafe(response);
      if (!response.ok) throw new Error(data.message || 'Upload failed');
      return { data, error: null };
  ...
  async createAppointmentWithPayment(payload) {
    try {
      console.log('MongoDB Service - Creating appointment with payment:', payload);
      console.log('MongoDB Service - API URL:', `${API_URL}/appointments/book-with-payment`);
      console.log('MongoDB Service - Headers:', this.getAuthHeaders());
      ...
      console.log('MongoDB Service - Response status:', response.status);
      const data = await this.parseJsonSafe(response);
      console.log('MongoDB Service - Response data:', data);
  ...
  async analyzeMRI(imageFile, patientId = null, notes = '') {
    try {
      console.log('MongoDB Service - Starting MRI analysis');
      ...
      const response = await fetch(`${API_URL}/mri/analyze`, {
        method: 'POST',
        headers: {
          ...(this.token && { Authorization: `Bearer ${this.token}` })
        },
      ...
      console.log('MongoDB Service - MRI Analysis response status:', response.status);
      const data = await this.parseJsonSafe(response);
      console.log('MongoDB Service - MRI Analysis response data:', data);

      if (!response.ok) throw new Error(data.message || 'Failed to analyze MRI');
      return { data, error: null };
    } catch (error) {
      console.error('MongoDB Service - MRI Analysis error:', error);
      return { data: null, error: { message: error.message } };
    }
  }

  async analyzeBatchMRI(imageFiles, patientId = null, notes = '') {
    try {
      console.log('MongoDB Service - Starting batch MRI analysis');
      ...
      const response = await fetch(`${API_URL}/mri/analyze-batch`, {
        method: 'POST',
        headers: {
          ...(this.token && { Authorization: `Bearer ${this.token}` })
        },
      ...
      console.log('MongoDB Service - Batch MRI Analysis response status:', response.status);
      const data = await this.parseJsonSafe(response);
      console.log('MongoDB Service - Batch MRI Analysis response data:', data);

      if (!response.ok) throw new Error(data.message || 'Failed to analyze MRI batch');
      return { data, error: null };
    } catch (error) {
      console.error('MongoDB Service - Batch MRI Analysis error:', error);
      return { data: null, error: { message: error.message } };
    }
  }

  async getMRIAnalysis(analysisId) {
    try {
      console.log('MongoDB Service - Getting MRI analysis:', analysisId);

      const response = await fetch(`${API_URL}/mri/analysis/${analysisId}`, {
        method: 'GET',
        headers: this.getAuthHeaders()
      });

      console.log('MongoDB Service - Get MRI Analysis response status:', response.status);
      const data = await this.parseJsonSafe(response);
      console.log('MongoDB Service - Get MRI Analysis response data:', data);

      if (!response.ok) throw new Error(data.message || 'Failed to get MRI analysis');
      return { data, error: null };
    } catch (error) {
      console.error('MongoDB Service - Get MRI Analysis error:', error);
      return { data: null, error: { message: error.message } };
    }
  }

  async getMRIAnalyses(page = 1, limit = 10) {
    try {
      console.log('MongoDB Service - Getting MRI analyses');

      const response = await fetch(`${API_URL}/mri/analyses?page=${page}&limit=${limit}`, {
        method: 'GET',
        headers: this.getAuthHeaders()
      });

      console.log('MongoDB Service - Get MRI Analyses response status:', response.status);
      const data = await this.parseJsonSafe(response);
      console.log('MongoDB Service - Get MRI Analyses response data:', data);

      if (!response.ok) throw new Error(data.message || 'Failed to get MRI analyses');
      return { data, error: null };
    } catch (error) {
      console.error('MongoDB Service - Get MRI Analyses error:', error);
      return { data: null, error: { message: error.message } };
    }
  }

  async getMRIModelInfo() {
    try {
      console.log('MongoDB Service - Getting MRI model info');

      const response = await fetch(`${API_URL}/mri/model-info`, {
        method: 'GET',
        headers: this.getAuthHeaders()
      });

      console.log('MongoDB Service - Get MRI Model Info response status:', response.status);
      const data = await this.parseJsonSafe(response);
      console.log('MongoDB Service - Get MRI Model Info response data:', data);

      if (!response.ok) throw new Error(data.message || 'Failed to get MRI model info');
      return { data, error: null };
    } catch (error) {
      console.error('MongoDB Service - Get MRI Model Info error:', error);
      return { data: null, error: { message: error.message } };
    }
  }
  ...
  async shareReportWithDoctor(reportId, doctorId, message = '') {
    try {
      const response = await fetch(`${API_URL}/reports/${reportId}/share-with-doctor`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
        body: JSON.stringify({ doctorId, message })
      });
      const data = await this.parseJsonSafe(response);
      if (!response.ok) throw new Error(data.error || data.message || 'Failed to share report');
      return { data: data.sharedReport, error: null };
    } catch (error) {
      return { data: null, error: { message: error.message } };
    }
  }

  // Get reports shared with the logged-in doctor
  async getDoctorSharedReports() {
    try {
      const response = await fetch(`${API_URL}/reports/shared/received`, {
        headers: this.getAuthHeaders()
      });
      const data = await this.parseJsonSafe(response);
      if (!response.ok) throw new Error(data.error || data.message || 'Failed to fetch shared reports');
      return { data: data.sharedReports || [], error: null };
    } catch (error) {
      return { data: [], error: { message: error.message } };
    }
  }

  // Acknowledge a shared report (doctor)
  async acknowledgeDoctorSharedReport(shareId) {
    try {
      const response = await fetch(`${API_URL}/reports/shared/${shareId}/acknowledge`, {
        method: 'PUT',
        headers: this.getAuthHeaders()
      });
      const data = await this.parseJsonSafe(response);
      if (!response.ok) throw new Error(data.error || data.message || 'Failed to acknowledge report');
      return { data, error: null };
    } catch (error) {
      return { data: null, error: { message: error.message } };
    }
