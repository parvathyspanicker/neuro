...
import { FaBrain, FaChevronDown, FaBars, FaSignOutAlt, FaPaperPlane, FaSearch, FaImage, FaPhoneSlash, FaMicrophone, FaMicrophoneSlash, FaVideo, FaVideoSlash, FaStop, FaTimes, FaFileAlt, FaDownload, FaFilePdf, FaFileWord, FaFile } from 'react-icons/fa';
...
export default function DoctorChat() {
  ...
  const sendImage = async () => {
    if (!imagePreview || !activeDoctorId) return;
    ...
    try {
      ...
      setMessages(prev => ({
        ...prev,
        [activeDoctorId]: (prev[activeDoctorId] || []).filter(m => m.id !== tempId)
      }));
    } finally {
  ...
  const sendDocument = async () => {
    if (!docPreview || !activeDoctorId) return;
    setLoadingMedia(true);
    ...
    try {
      ...
      if (up.data?.url) {
        ...
        await mongodbService.sendMediaMessage(activeDoctorId, docUrl, 'document', text);
        // Update temp msg with real URL
        setMessages(prev => ({
          ...prev, [activeDoctorId]: (prev[activeDoctorId] || []).map(m =>
      ...
      setMessages(prev => ({ ...prev, [activeDoctorId]: (prev[activeDoctorId] || []).filter(m => m.id !== tempId) }));
    } finally {
      setLoadingMedia(false);
    }
  };
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
          ...
          const up = await mongodbService.uploadMedia(dataUri, 'auto'); // Cloudinary treats audio as video usually
          if (up.data?.url) {
            await mongodbService.sendMediaMessage(activeDoctorId, up.data.url, 'audio');
          } else {
            // remove if failed
            setMessages(prev => ({
              ...prev,
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <div className="relative">
              <button onClick={() => setShowUserDropdown(!showUserDropdown)} className="flex items-center gap-2 text-gray-600 hover:text-blue-600 transition-colors">
                ...
                <span className="text-sm">{user?.firstName ? `${user.firstName} ${user.lastName || ''}`.trim() : (user?.user_metadata?.full_name || user?.email || 'User')}</span>
        ...
        <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-lg overflow-hidden max-w-md w-full shadow-2xl relative">
            ...
            <div className="p-4 bg-white flex justify-end gap-3 border-t">
              ...
              <button onClick={sendDocument} disabled={loadingMedia} className="px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 flex items-center gap-2">
                {loadingMedia ? 'Sending...' : 'Send'}
                <FaPaperPlane />
              </button>
            </div>
